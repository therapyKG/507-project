...
(module+ main
  ...
  (pipeline
   (train (model (bits 32) (linear 784 128 relu) (linear 128 10))
          on (dataset "MNIST-A" 10000 (bits 32) 784))
   (train (model (bits 32) (linear 784 64 relu) (linear 64 10))
  ...
  (pipeline
   ...
   (train (model (bits 32) (linear 1024 1024 relu) (linear 1024 10))
          on (dataset "VectorData" 2000 (bits 32) 1024))
   with (vram 50)) 
  
  (printf "\nTest 3: Impossible Pipeline (OOM)\n")
  (pipeline
   (train (model (linear 1000 1000)) on (dataset "A" 100 (bits 32) 1000))
   (train (model (linear 1000 1000)) on (dataset "B" 100 (bits 32) 1000))
   with (vram 20))

  (printf "\nTest 4: Auto-Partitioning Strategy\n")
  (pipeline
   (train (model (bits 32) (linear 1000 1000))
  ...
  (printf "\nTest 5: Fine-Grained Component Liveness\n")
  ...
  (printf "\nTest 6: Extended Config Syntax\n")
  (define-component big-model (bits 32) (linear 1500 1300))
  (define-component small-model (bits 32) (linear 100 100))
